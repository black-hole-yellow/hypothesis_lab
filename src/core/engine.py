...
class LabEngine:
    ...
    def run_hypothesis(self, hypothesis):
        """Universal Trade Manager Powered by JSON"""
        current_day = None
        ...
        sl_atr_mult = exec_rules.get('default_sl_atr_multiplier', 1.0)
        ...
        for index, row in self.df.iterrows():
            ...
            # 2. EVALUATE SIGNAL (Фильтры по тренду применяются внутри парсера JSON)
            triggers_before = len(hypothesis.triggers)
            hypothesis.evaluate_row(row, index)

            # 3. GLOBAL GUARDS & INIT
            if len(hypothesis.triggers) > triggers_before:
                ...
                # Защита от переторговки (Управляется через JSON)
                limit_allows = False
                ...
                # ИНИЦИАЛИЗАЦИЯ
                entry_price, sl_price = row['Close'], None
